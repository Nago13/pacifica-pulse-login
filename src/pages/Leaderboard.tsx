...
import { Home, Trophy, BarChart3, User, Flame, Crown } from "lucide-react";
...
const otherPlayers: Player[] = Array.from({ length: 17 }, (_, i) => ({
  rank: i + 4,
...
const currentUser: Player = { rank: 142, name: "Pedro", initials: "PE", league: "Ouro", streak: 5, trophies: 847 };
    ...
    <div className="min-h-screen bg-ocean-dark font-dm-sans flex flex-col">
        ...
        <div className="w-full max-w-[520px] mx-auto flex flex-col gap-5">
            ...
            {tabs.map((t) => (
              <button
                ...
                className={`pb-2.5 text-sm font-medium transition-all ${
                  activeTab === t
                    ? "text-foreground border-b-2 border-pacific"
                    : "text-ocean-muted hover:text-foreground"
                }`}
              >
                {t}
              </button>
              ...
              <button
                ...
                className={`shrink-0 px-3.5 py-1.5 rounded-full text-xs font-medium transition-all ${
                  activeLeague === l
                    ...
                    : "bg-card-surface text-ocean-muted hover:text-foreground"
                }`}
              >
                {l}
              </button>
            ...
            {podiumOrder.map((p, i) => (
              <div key={p.rank} className={`flex flex-col items-center gap-2 ${podiumDelays[i]}`}>
                <div className="relative">
                  <div className="w-12 h-12 rounded-full bg-ocean-button flex items-center justify-center text-foreground font-bold text-sm border-2 border-pacific/30">
                    {p.initials}
                  </div>
                ...
                <div
                  className={`w-20 ${podiumHeights[i]} rounded-t-lg flex items-start justify-center pt-2`}
                  style={{
                    background: i === 1
          ...
          <div className="rounded-[12px] bg-ocean-button p-3 flex items-center gap-3" style={{ border: "1px solid hsl(195,74%,64%)" }}>
            <div className="flex flex-col">
              <span className="text-ocean-muted text-[11px]">Sua posição</span>
            </div>
            <span className="text-pacific text-xl font-bold">#{currentUser.rank}</span>
            <div className="w-8 h-8 rounded-full bg-ocean-dark flex items-center justify-center text-foreground text-xs font-bold border border-pacific/30">
              {currentUser.initials}
            </div>
            <span className="text-foreground text-sm font-medium">{currentUser.name}</span>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${leagueBadgeColors[currentUser.league]}`}>
              {currentUser.league}
            </span>
            <span className="text-foreground text-sm font-bold ml-auto">{currentUser.trophies}</span>
            ...
            {otherPlayers.map((p, i) => (
              <div key={p.rank}>
                <div className="flex items-center gap-3 px-4 py-3">
                  <span className="text-ocean-muted text-sm font-medium w-7 text-right shrink-0">
                    {p.rank}
                  </span>
                  <div className="w-8 h-8 rounded-full bg-ocean-dark flex items-center justify-center text-foreground text-[10px] font-bold shrink-0">
                    {p.initials}
                  </div>
                  <div className="flex items-center gap-1.5 flex-1 min-w-0">
                    <span className="text-foreground text-sm font-medium truncate">{p.name}</span>
                    <span className={`shrink-0 px-1.5 py-0.5 rounded-full text-[9px] font-bold ${leagueBadgeColors[p.league]}`}>
                      {p.league}
                    </span>
                  </div>
                  ...
                  <span className="text-foreground text-sm font-bold w-12 text-right shrink-0">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-nav-bg flex items-center justify-around py-3 z-20" style={{ borderTop: "1px solid rgba(92,200,232,0.1)" }}>
        {[
          ...
          { icon: Trophy, label: "Troféus", active: false, path: "/dashboard" },
          { icon: BarChart3, label: "Ranking", active: true, path: "/leaderboard" },
          { icon: User, label: "Perfil", active: false, path: "/profile" },
        ].map((item) => (
