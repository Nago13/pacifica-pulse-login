...
import { Check, X, Flame, Trophy, Package, BarChart3 } from "lucide-react";
...
const PredictionResult = () => {
  ...
  return (
    <div className="min-h-screen bg-ocean-dark font-dm-sans flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      {/* Main Card */}
      <div
        className={`relative z-10 w-full max-w-md rounded-[16px] bg-card-surface p-6 sm:p-8 animate-result-enter border-2 ${borderColor}`}
      >
        {/* Icon */}
        ...
        <div className="text-center mb-5">
          <CountUp
            target={acertou ? MULTIPLIER_TOTAL : TROPHY_TARGET}
        ...
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <span className="text-foreground text-sm font-medium">
            {acertou ? "Streak: 5 dias" : "Streak zerado"}
          </span>
          {acertou && (
            <span className="px-2 py-0.5 rounded-full bg-warning text-ocean-dark text-[10px] font-bold">
              Novo recorde!
            </span>
          )}
        </div>
        ...
        {acertou && (
          <p className="text-success text-xs text-center mb-6">
            Multiplicador 1.5× aplicado → +37 troféus
          </p>
        )}
        ...
        {/* Share Preview Card */}
        <div
          className="rounded-[12px] bg-ocean-dark p-4 mb-5"
        ...
        >
          ...
          <p className="text-foreground text-sm font-medium mb-1">
            {acertou ? "✅ Acertei! BTC +2.4%" : "📊 Errei, mas sigo firme!"}
          </p>
        ...
        >
          <XIcon />
          Postar no X e ganhar +30 troféus
        </a>

        <button className="w-full h-11 rounded-[12px] flex items-center justify-center gap-2 text-ocean-muted font-medium text-sm transition-all duration-200 hover:text-foreground border border-ocean-muted/30">
          <BarChart3 size={16} />
          Ver leaderboard
        </button>
      </div>
